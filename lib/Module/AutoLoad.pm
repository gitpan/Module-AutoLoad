...
This module is the only one on CPAN that is never actually intented to be installed.
...
1. Network access is required in order to download the modules from CPAN.
...
2. Also, because of all the network traffic used, this module can be quite slow,
...
3. Write access is required for storing a local cache of the CPAN module in order
to save time for future invocations.
(See AUTOLOAD_LIB below for more details.)

4. This only works for simple Pure Perl CPAN modules.
If you use modules with XS or bytecode, you will probably have to truly install it first.

5. You must always load the main distribution module first,
...
6. And for the same reason, those crazy distribution names that aren't really a module
are more difficult to load on the fly. One workaround is with eval.

  # For example, Mail::Map is part of the MailTools distribution.
...
You can use AUTOLOAD_SRC to specify the mapper engine is to ask where the latest location of the module is.

  # For example
  BEGIN { $ENV{AUTOLOAD_SRC} = "https://metacpan.org/release"; }
